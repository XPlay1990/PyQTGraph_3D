...
class Surface3D_Graph:
    ...
    def __init__(self):
        ...
        self.p4 = gl.GLSurfacePlotItem(self.x, self.y, shader='heightColor', computeNormals=False,
                                  smooth=False)  # smooth true = faster; dont turn on computenormals
        self.p4.shader()['colorMap'] = np.array([0.01, 40, 0.5, 0.01, 40, 1, 0.01, 40, 2])  # lut
        # p4.setDepthValue(1000)
        # p4.setGLOptions('opaque')
        self.data = np.zeros((self.widthOfData, self.numberOfData), dtype=int)
    ...
    def updateSelf(self):
        ...
        self.data = np.concatenate((self.data, newValues))
        self.p4.setData(z=self.data)
